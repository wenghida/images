...
import { Upload, X, Image as ImageIcon, Loader2, Check, Sparkles, Plus } from 'lucide-react';
...
export default function ImageUploader() {
  ...
  return (
    <section id="upload" className="section-padding relative overflow-hidden bg-gray-50">
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg p-8 md:p-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
              <div>
                ...
                <div 
                  ...
                  className={`border-2 border-dashed rounded-xl p-6 h-64 flex flex-col items-center justify-center cursor-pointer transition-all ${
                    isDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-400 hover:bg-blue-50/50'
                ...
                {preview && (
                  <div className="mt-8">
                    <h3 className="text-xl font-bold mb-6 flex items-center">
                      <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center mr-3 text-sm font-bold">2</div>
                      选择风格
                    </h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {styles.map((style) => (
                        ...
                        >
                          {selectedStyle === style.id && <Check size={16} className="inline-block mr-1" />}
                          {style.name}
                        </button>
                      ))}
                    </div>
              ...
              <div>
                <h3 className="text-xl font-bold mb-6 flex items-center">
                  <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center mr-3 text-sm font-bold">3</div>
                  生成结果
                </h3>
                  ...
                  {generatedImage ? (
                    <div className="relative w-full h-full">
                      ...
                      <div className="absolute bottom-3 right-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white py-1.5 px-4 rounded-full text-xs shadow-md flex items-center">
                        ...
                        {styles.find(s => s.id === selectedStyle)?.name || '卡通风格'}
                      </div>
                    </div>
                  ) : (
                    <div className="text-center">
                      ...
                      <p className="text-gray-600 font-medium">
                        {isGenerating ? '正在生成图片...' : '生成的图片将显示在这里'}
                      </p>
                      {isGenerating && (
